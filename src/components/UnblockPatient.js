...
const bookAmb=(item)=>{
    if (item.Status =="lock") {
     ...
     axios.post('https://localhost:44328/api/Patient/update',data)
         .then((result)=>{
           alert(result)
           document.getElementById('bkamb').innerHTML="UnBlock"
         })
         .catch((err)=>{
           alert(err);
...
useEffect(() => {
    fetch("https://localhost:44328/api/BlockPatients")
    .then((res)=>{
        return res.json()
    })
    .then((data)=>
                        ...
                        { doctors &&
                            doctors.map(item=>(
                                <tr key={item.Id}>
                                    ...
                                    <a id='bkamb' onClick={()=>bookAmb(item)} className='btn btn-success'>UnBlock </a>
